...
from lythossettle import forms, render, summary
from lythossettle.web import server as server_module
from lythossettle.web.session import Session
from lythossettle.web.strings import shell_strings
...
def test_analyse_returns_cards_text_and_figures(session):
    ...
    assert f"{result['total']:.1f}" in result["text"]
    assert result["figures"] == render.PLOT_KEYS
    assert set(result["points"]) == {"center", "char", "edge", "corner"}
...
def test_a_refused_analysis_says_why_in_the_session_language():
    ...
    values["Df"] = 99.0
    s = Session(lang="tr")
    with pytest.raises(ValueError, match="Temel tabanı"):
        s.analyse(values)


def test_warnings_reach_the_page_translated():
    values = forms.defaults()
    values["q"] = 10.0                            # less than the excavated overburden
    result = Session(lang="tr").analyse(values)
    assert any("dengelenmiş" in w for w in result["warnings"])


@pytest.mark.parametrize("kind", render.PLOT_KEYS)
...
def test_the_language_reaches_results_and_figures(lang):
    ...
    assert TRANSLATIONS[lang]["card_total"] == result["cards"][0]["title"]
    assert TRANSLATIONS[lang]["res_title"] in result["text"]
...
def test_a_project_file_round_trips_through_the_session(session):
    ...
    assert project["format"] == "lythos-settle"
    loaded = session.load_project(project)
    assert loaded["ok"] and loaded["values"]["B"] == 9.5


def test_another_programs_file_is_refused(session):
    with pytest.raises(ValueError):
        session.load_project({"format": "lythos-spwa"})


def test_a_study_runs_reports_progress_and_can_be_read_back():
    s = Session()
    values = forms.defaults()
    values.update(study_method="lhs", study_n=30, study_variables=[
        {"path": "foundation.q", "label": "q", "mode": "dist", "dist": "normal",
         "mean": 100.0, "cov": 0.1, "min": 0, "max": 0, "n_points": 5},
        {"path": "soil_profile.2.Cc", "label": "Cc", "mode": "range", "min": 0.25,
         "max": 0.4, "dist": "normal", "mean": 0, "cov": 0.1, "n_points": 5}])
    s.analyse(values)
    ...
    for view in payload["views"]:
        png = s.plot("study", view, payload["outputs"][0]["value"])
...
def test_the_page_and_its_assets_are_served(http):
    ...
    assert "<title>Lythos Settle</title>" in page
...
def test_meta_and_state_are_served_as_json(http):
    ...
    assert meta["app"] == "Lythos Settle" and meta["version"]
...
def test_an_analysis_over_http_gives_results_and_a_figure(http):
    ...
    png = get(f"{http}/api/plot?target=analysis&kind=settlement_depth", raw=True)
    assert png.startswith(b"\x89PNG")


def test_the_report_comes_back_as_a_download(http):
    post(f"{http}/api/analyse", {"values": get(f"{http}/api/meta")["defaults"]})
...
def test_the_project_file_comes_back_as_a_download(http):
    ...
    assert json.loads(body)["format"] == "lythos-settle"
    assert "project.settle" in headers["Content-Disposition"]
...
def test_switching_the_language_returns_the_translated_schema(http):
    ...
    assert turkish["schema"]["project"]["groups"][0]["title"] == "Proje"
    english = post(f"{http}/api/language", {"lang": "en"})
    assert english["schema"]["project"]["groups"][0]["title"] == "Project"
...
def test_a_study_survives_analysing_the_same_inputs_again():
    """Changing the language re-runs the analysis; the study must not be lost."""
    s = Session()
    values = forms.defaults()
    values["study_variables"] = [{"path": "foundation.q", "mode": "range", "min": 90,
                                  "max": 110, "n_points": 3}]
    values["study_method"] = "oat"
    s.analyse(values)
    assert s.start_study(values)["ok"]
    for _ in range(300):
        if s.state()["job"] != "running":
        ...
        time.sleep(0.05)
    s.set_language("tr")
    relabelled = dict(values, study_variables=[dict(values["study_variables"][0], label="Temel · q")])
    s.analyse(relabelled)
    assert s.study_payload()["ok"]
    changed = dict(values, q=120.0)
    s.analyse(changed)
    assert not s.study_payload()["ok"]
