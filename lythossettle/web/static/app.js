"use strict";
/* Lythos Settle — the browser interface.
 *
...
/* Soil columns that only mean something in a cohesive layer */
const CLAY_ONLY = ["Cc", "Cr", "e0", "OCR", "cv", "Calpha", "drainage"];
...
function renderFoundationForm() {
  renderForm($("foundationForm"), S.meta.schema.foundation.groups);
}
...
function renderTable(name) {
  ...
  rows.forEach((row, index) => {
    ...
    for (const column of columns) {
      ...
      const off = granular && CLAY_ONLY.includes(column.key);
      input.disabled = off;
...
function updatePicker() {
  ...
  if (S.module === "inputs") {
    ...
    if (!keys.includes(S.figure)) S.figure = keys[0] || "schematic";
...
function renderView() {
  ...
  if (S.module === "inputs") {
    ...
    if (which === "summary") {
      ...
      parts.push(plotImage("analysis", "schematic"));
      parts.push(plotImage("analysis", "time"));
...
async function loadStudy() {
  try {
    ...
    if (!data.views.includes(S.figure)) S.figure = data.views[0] || "hist";
    S.plotVersion += 1;
    renderTabs();
    renderView();
  } catch (error) {
...
async function downloadReport() {
  ...
  try {
    ...
    await download("/api/report", { format }, `lythossettle_report.${format}`);
...
async function exportStudy(format) {
  try {
    await download("/api/export-study", { format }, `lythossettle_study.${format}`);
    status(T("saved"));
  } catch (error) {
    status(`${T("error")}: ${error.message}`, true);
  }
...
async function saveProject() {
  try {
    await download("/api/project", { values: S.values }, "project.settle");
    status(T("saved"));
  } catch (error) {
    status(`${T("error")}: ${error.message}`, true);
  }
...
async function applyTheme(theme) {
  ...
  try {
    localStorage.setItem("lythossettle-theme", theme);
...
function storedTheme() {
  try {
    return localStorage.getItem("lythossettle-theme");
  } catch {
    return null;
  }
}
