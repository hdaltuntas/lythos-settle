...
The distribution is named `lythossettle`; so are the package you import and the
directory in the repository. The command you type is `lythos-settle`.
...
with one scoped to `lythossettle`. Put it in `~/.pypirc`:
...
1. Raise `version` in `pyproject.toml` and `__version__` in
   `lythossettle/__init__.py`; the test suite checks that the two agree.
...
5. Check what was actually published, in an empty environment:
   ...
   /tmp/check/bin/pip install lythossettle
   /tmp/check/bin/lythos-settle example -o /tmp/check.settle
   /tmp/check/bin/lythos-settle run /tmp/check.settle -o /tmp/check.pdf
   /tmp/check/bin/lythos-settle web --no-browser        # then open the address
   ```
...
pip install --index-url https://test.pypi.org/simple/ \
            --extra-index-url https://pypi.org/simple/ lythossettle
```
...
`lythossettle/web/static/*` is declared as package data in `pyproject.toml`.
...
```bash
python -c "import zipfile; print(zipfile.ZipFile('dist/lythossettle-0.1.0-py3-none-any.whl').namelist())"
```
